...
def resume_train_state(model, checkpoint, optimizers, schedulers, accelerator):
    try:
        ...
        optimizers = load_param(base_path, optimizers, 'optimizer')
        schedulers = load_param(base_path, schedulers, 'scheduler')
        ...
        return model, optimizers, schedulers, 0, 0, 0, -1, {}
...
def load_param(base_path, param_dict, type='optimizer'):
    num = 0
    for key in param_dict.keys():
        if num == 0:
            ...
            add = f'_{num}'
        param_dict[key].load_state_dict(torch.load(base_path + f"/{type}{add}.bin"))  
        num += 1
    return param_dict
